...
void ofApp::setup(){
	...
	_keyMapper.getParameters().setName("Mapper parameters");
	_parameters.add(_keyMapper.getParameters());
	_parameters.add(_mappableParameters);

	_gui.setup(_parameters);

	_keyMapper.addParameter(_mappableBoolParameter);
	_keyMapper.addParameter(_mappableFloatParameter);
	_keyMapper.addParameter(_mappableIntParameter);
	_keyMapper.addParameter(_mappableVoidParameter);
}
...
void ofApp::keyReleased(int key){
	switch(key){
	 case 'l':
		 _keyMapper.loadMapping("mapping.json");
		 break;

	 case 's':
		 _keyMapper.saveMapping("mapping.json");
		 break;
	}
